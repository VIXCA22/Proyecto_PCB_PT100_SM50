...
La restricción de adquisición más importante fue el rango de entrada de la BoomBox/B-Box. La documentación de imperix reporta entradas analógicas diferenciales de $\pm5$ V y alimentación de sensores de $\pm15$ V limitada a 100 mA por el cable analógico \cite{imperix}. Con ese dato, el diseño no se planteó para llegar exactamente a $\pm5$ V, sino para quedar cerca y conservar margen frente a tolerancias, offset y calibración.
...
El diseño no se hizo como un esquemático grande de una sola vez. Se trabajó por etapas, llamadas durante el desarrollo ``ciudades'', para separar las señales de milivoltios de las zonas de alimentación, protección y conectores. Esta decisión sigue el criterio de atenuación de ruido de separar fuentes de interferencia, retornos y señales sensibles para reducir acoplos capacitivos, inductivos y por impedancia común \cite{cardenas_ruido}; por eso ayuda a proteger rutas como la del SM-50, que entrega hasta 30 mV antes de amplificar.
...
La selección de componentes siguió la función de cada etapa. El criterio común fue evitar componentes ``de relleno'': cada integrado, protección o pasivo debía ayudar a cumplir una restricción de la BoomBox, del sensor o de la señal medida. La interfaz con la BoomBox/B-Box se resolvió con conectores RJ45 8P8C blindados y cable FTP porque el equipo entrega entradas diferenciales, tierra y rieles de alimentación para sensores por el cable analógico \cite{imperix}; por eso la tarjeta separa alimentación, retorno y señales débiles desde el conector.

En el canal PT100, el REF3012 de 1.25 V se usó como referencia de la fuente de corriente, de modo que $I=V_{\text{ref}}/R_{\text{SET}}$ no dependiera de un divisor simple ni de variaciones directas del riel \cite{ref30}. El OPA2188 se eligió como operacional de precisión porque es zero-drift y presenta bajo offset, lo cual importa cuando la variación útil del RTD está en milivoltios \cite{opa2188}. El INA826 se mantuvo como amplificador diferencial porque mide señales pequeñas con rechazo de modo común y permite fijar la ganancia con una sola resistencia, $G=1+49.4~\text{k}\Omega/R_G$ \cite{ina826}.

En el canal SM-50, el TPS7A4901 genera la excitación de 10 V del puente desde el riel positivo disponible. Se escogió por ser un regulador de alto voltaje, bajo ruido y buena PSRR, adecuado para no inyectar ruido de alimentación en una señal de puente que antes de amplificar está en el orden de milivoltios \cite{tps7a49}, \cite{interface}. El regulador 78L05 se dejó como referencia local de 5 V para desacoplar bloques auxiliares del riel de la BoomBox; su salida fija, limitación de corriente y protección térmica lo hacen suficiente para una alimentación local sencilla \cite{lm78l}.

La protección también se justificó desde la forma real de uso. Como la BoomBox limita la alimentación de sensores a 100 mA, se agregó un PPTC 1812L050/30PR antes de distribuir rieles para tener protección resettable contra sobrecorriente \cite{littelfuse1812}. Las líneas hacia conectores se protegieron con TPD4E1U06 y SMF12CT1G: el primero aporta protección ESD de baja capacitancia para líneas de señal \cite{tpd4e1u06}, y el segundo funciona como arreglo TVS para transitorios en zonas expuestas \cite{smf12}. Finalmente, los trimmers multivuelta no se usaron para improvisar la ganancia, sino para compensar offset, ruido y dispersión de laboratorio; las redes RC limitan ancho de banda antes de la adquisición y los desacoplos de 100 nF/1 $\mu$F/10 $\mu$F dan reserva local de carga \cite{bourns3296}.
...
El stackup del proyecto se dejó como tarjeta de 1.6 mm con cobre externo de 35 $\mu$m, equivalente al cobre comercial típico de 1 oz. En la revisión del layout y del paquete Gerber actualizado se confirmaron pistas rutadas de 0.20, 0.25, 0.30, 0.40, 0.50 y 0.60 mm. Los anchos principales del diseño están entre 0.25 y 0.60 mm; el valor de 0.20 mm aparece solo en dos segmentos puntuales y el mayor ancho utilizado en fabricación es 0.60 mm. La documentación oficial de KiCad indica que estas reglas de diseño gobiernan el ruteo, la verificación DRC y los anchos disponibles durante el trazado \cite{kicad_pcbnew}.

La decisión no fue solo por espacio. El criterio eléctrico se revisó con el modelo IPC-2221 para pistas externas, usando 1 oz de cobre y 10 $^\circ$C de aumento térmico como caso conservador \cite{digikey_trace}. Esa corriente calculada no representa la corriente recibida por el circuito, sino un margen térmico de referencia frente al peor caso real: 0.5 mA en el PT100, 28.6 mA en el puente SM-50 y 100 mA como límite de alimentación de sensores de la B-Box. Por eso las pistas de 0.50 y 0.60 mm no se eligieron porque la corriente lo exigiera al límite, sino para reducir caída de tensión, mejorar robustez en conectores y separar físicamente potencia/retorno de las señales de milivoltios. La tabla completa de anchos y márgenes queda en el Anexo A.
...
\begin{table}[H]
\caption{Anchos de pista confirmados en fabricación y margen frente a 100 mA}
\label{tab:pistas}
\centering
\tiny
\setlength{\tabcolsep}{2pt}
\begin{tabular}{@{}p{0.14\linewidth} p{0.34\linewidth} p{0.18\linewidth} p{0.20\linewidth}@{}}
\toprule
Ancho & Uso en la PCB & Margen vs. 100 mA & Caída, 100 mA/50 mm \\
\midrule
0.20 mm & Segmentos puntuales confirmados & 7.4$\times$ & 12.31 mV \\
0.25 mm & Señales analógicas; ancho dominante & 8.8$\times$ & 9.85 mV \\
0.30 mm & Ajustes y salidas locales & 10.0$\times$ & 8.21 mV \\
0.40 mm & Referencias y tramos intermedios & 12.3$\times$ & 6.16 mV \\
0.50 mm & Rieles protegidos y conectores & 14.5$\times$ & 4.93 mV \\
0.60 mm & Alimentación, retorno y pantallas con margen extra & 16.5$\times$ & 4.10 mV \\
\bottomrule
\end{tabular}
\end{table}
...
\bibitem{cardenas_ruido} R. Cárdenas Dobson, ``Atenuación de ruidos en circuitos electrónicos,'' apuntes del curso de proyecto, Universidad de Magallanes.
...
\bibitem{labces_prev} LabCES, ``Caracterización de sensores y datos experimentales previos del banco de motor de inducción,'' documentación interna usada como antecedente experimental previo, 2026.
...
\bibitem{lm78l} Texas Instruments, ``LM78L 100-mA Fixed Output Linear Regulator,'' hoja de datos. [En línea]. Disponible: \url{https://www.ti.com/lit/ds/symlink/lm78l.pdf}
\bibitem{littelfuse1812} Littelfuse, ``1812L Series Surface Mount Resettable PPTC,'' hoja de datos. [En línea]. Disponible: \url{https://www.littelfuse.com/assetdocs/resettable-ptcs-1812l-datasheet?assetguid=ca5c80cb-504e-4a8a-8e74-0107520a1717}
\bibitem{tpd4e1u06} Texas Instruments, ``TPD4E1U06 Quad-Channel High-Speed ESD Protection Device,'' hoja de datos. [En línea]. Disponible: \url{https://www.ti.com/lit/gpn/TPD4E1U06}
\bibitem{smf12} onsemi, ``SMF05CT1G, SMF12CT1G, SMF15CT1G, SMF24CT1G ESD Protection Diode Array,'' hoja de datos. [En línea]. Disponible: \url{https://www.onsemi.com/download/data-sheet/pdf/smf05c-d.pdf}
